...
with tab2:
    ...
    conn = get_connection()
    df_nilai = pd.read_sql("WITH ranked_penilaian AS (SELECT
        id,
        nim,
        inisial,
        accuracy,
        ...
        ROW_NUMBER() OVER (
            PARTITION BY nim
            ORDER BY accuracy DESC, timestamp ASC
        ) AS rownum
    FROM penilaian
),
final_ranking AS (
    SELECT
        *,
        RANK() OVER (ORDER BY accuracy DESC, timestamp ASC) AS ranking
    FROM ranked_penilaian
    WHERE rownum = 1
) SELECT * FROM final_ranking",con)
